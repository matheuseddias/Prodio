...
Três migrations (limite de 400 linhas): `20260926000100_import_catalog.sql` (helpers e validação do payload), `20260926000200_import_catalog_cadastros.sql` (fornecedores, insumos, produtos e apelidos) e `20260926000300_import_catalog_fichas.sql` (vínculos, fichas e a RPC pública). Nenhuma tabela nova.

RPC: `import_catalog(p_tenant_id, p_payload jsonb, p_dry_run boolean default true) → jsonb` — SECURITY DEFINER, `search_path = ''`, primeira instrução `assert_member(admin)`; `revoke` de public/anon, `grant` a authenticated. O payload é o `PayloadImportacao` de `@prodio/core/importacaoEs` (montado no navegador a partir do backup do ES; o arquivo nunca vai ao servidor): `{versao: 1, origem: 'es', fornecedores, insumos, produtos, vinculos, fichas}`, sem nenhum uuid. Devolve `{simulacao, exemplo: {produtos, insumos, fornecedores}, contagens: {fornecedor|insumo|produto|apelido|vinculo|ficha: {novos, atualizados, iguais, problemas}}, linhas: [{entidade, chave, situacao: novo|atualizado|problema|aviso, campos?, mensagem?}]}` (iguais só nas contagens). Chaves das linhas: fornecedor = CNPJ, insumo = SKU, produto = SKU principal, apelido = sku_externo, vínculo = `cnpj|sku`, ficha = SKU do produto.
...
- **Ordem, numa transação**: `pg_advisory_xact_lock` por tenant → conta os dados de exemplo pelos ids fixos do seed (`a0…`, `b0…`, `c0…`); gravar com exemplo presente é `55000` → fornecedores → insumos → produtos e apelidos → vínculos → fichas → checagem de ciclo nas fichas ativas que mudaram (aborta tudo) → uma linha `audit_log` (`entidade = 'import_catalog'`, `acao = 'importacao_es'`, só contagens) quando algo mudou.
...
- Funções internas (sem EXECUTE para ninguém, na lista 5 do teste 0010): `import_catalog_validate`, `import_catalog_suppliers`, `import_catalog_materials`, `import_catalog_products`, `import_catalog_links`, `import_catalog_boms` e os helpers `import_catalog_txt|num|chaves|sku|opc|calc|linha|contagem`.
- Testes: `0013_import_catalog` (fluxo, idempotência, papéis, tenants, payload inválido), `0014_import_catalog_conflitos` (conflitos e dados de exemplo), `0015_import_catalog_carga` (400 insumos/produtos/fichas × 8 linhas abaixo de 6 s; o Supabase corta o authenticated em 8 s), `0016_limpar_exemplo` (trava do `limpar_exemplo.sql`) e a parte (c) do `db:test:api`.
